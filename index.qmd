...
- Employed idea of inference on **conditional effects with IPW**
...
- Under identification assumptions, $\mathbb{E}[\hat{Y_i^*}|X_i = x] = \tau(x)$
...
1. Transform $Y_i$ by **AIPW** with consistent **parametric** estimators $\hat\mu_0(w, x)$ and $\hat{e}_0(x)$ to obtain $\hat{Y}^*_i$
...
$$
\sqrt{n}(\hat\beta - \beta_0) = \left(\frac1n \sum_{i = 1}^{n} X_i'X_i\right)^{-1} \left(\frac{1}{\sqrt{n}}\sum_{i = 1}^{n} \left(X_i\epsilon_i + \underbrace{X_i'\frac{\partial\hat{Y}^*_i}{\partial \gamma} (\hat\gamma - \gamma_0)}_{\color{red}{\frac1n \sum \cdot\rightarrow 0 \text{ because of  AIPW}}}\right)\right)
$$
